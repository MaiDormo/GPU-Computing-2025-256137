...
int main(int argc, char ** argv) { 
    ...
    const int block_num = (nnz + BLOCK_SIZE - 1) / (BLOCK_SIZE);
    ...
    // --- Warmup Run ---
    value_parallel_sequential_spmv<<<block_num, BLOCK_SIZE>>>(
        d_csr.values, d_csr.row_pointers, d_csr.col_indices, d_vec, d_res, d_csr.num_non_zeros, n
    );
